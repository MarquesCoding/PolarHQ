...
<img src="apps/web/public/logo.png" width="92" alt="Vault logo" />

# Vault

**Your digital life, under your control.**

An open-source, self-hosted, **end-to-end encrypted** explorer that you run on your
own server. Files, photos, documents, spreadsheets and boards — every kind of thing
you own, in **one** private place. (A PolarHQ project.)
...
<img src="image.png" width="100%" alt="Vault" />
...
<p>
  ...
  <img src="https://img.shields.io/badge/TanStack%20Router-EF4444?style=flat-square&logo=reactrouter&logoColor=white" alt="TanStack Router" />
...
- **Monorepo** — Turborepo + pnpm workspaces; ~all UI/logic lives in shared, framework-agnostic packages
- **Web** — a thin Vite + React 19 + TanStack Router shell over `@polarhq/interface`; Tailwind v4, Redux Toolkit, TanStack Query
- **Desktop** — Tauri 2 wrapping the same frontend (`apps/desktop`, bundle id `vault.polarhq.app`)
- **API** — Hono + tRPC, Drizzle ORM, better-auth, a dumb `/ws` relay for collaboration; the media + backup workers run in-process (`RUN_WORKERS`)
...
- **Mobile** — React Native + Swift client (planned), reusing `@polarhq/core` + `@polarhq/i18n`
...
# 5. Start everything (web, api with in-process workers, marketing)
pnpm dev
```
...
```
apps/
  web/         Thin Vite + TanStack Router shell (src/routes + main.tsx)
  desktop/     Tauri 2 desktop wrapper (vault.polarhq.app)
  api/         Hono + tRPC API, auth, storage, collaboration relay, in-process workers
  marketing/   polarhq.app landing site (Next.js)
packages/
  interface/   The whole UI — screens, components, store, hooks (renderer-agnostic React)
  vault/       Domain layer — drive/e2e/docs/photos, doc editor, office, CLIP embedder
  sdk/         Backend client — configureSdk + apiClient + network + auth + live events
  core/        Framework-agnostic primitives — crypto, formatting, changelog
  i18n/        Translations + i18n config/provider
...
[AGPL-3.0](LICENSE.md) — Vault is free software. You can self-host, modify and redistribute it;
