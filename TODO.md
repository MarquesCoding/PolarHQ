...
- [ ] **Chunked Phase 2 remainder:** resume-on-failure (note: secretstream state can't be restored mid-stream without replaying chunks to advance it), **in-viewer playback for large encrypted videos** (the lightbox still whole-buffer-decrypts the original to an object URL → OOM for multi-GB; needs MediaSource + streaming decrypt), and **iOS secretstream support** (chunk-uploaded files aren't readable on iOS until then — crypto-parity gate).
