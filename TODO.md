...
- [ ] **Chunked Phase 2 remainder:** resume-on-failure (note: secretstream state can't be restored mid-stream without replaying chunks to advance it), **streaming download + in-viewer playback for large encrypted videos** (Photos download/lightbox still whole-buffer → OOM for multi-GB), and **iOS secretstream support** (chunk-uploaded files aren't readable on iOS until then — crypto-parity gate).
