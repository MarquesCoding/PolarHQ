...
- [~] **Smart/saved views in the sidebar**: **Recents** (`/drive/recent`) and **File Kinds** (`/drive/kind/[kind]` — images/videos/documents/audio/archives) now ship as sidebar entries. New `GET /api/v1/drive/library?view=recent|kind&kind=` returns a flat, cross-folder file listing; Browser gained an optional parentless `source` so the smart views reuse all of its open/selection/inspector/context-menu behaviour (folder chrome — upload/new-folder/drag-to-parent/breadcrumbs — auto-disabled). _Remaining: Favorites (needs a star/favourite flag on nodes) and saved searches._
