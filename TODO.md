...
- [ ] **Chunked Phase 2 remainder:** resume-on-failure (note: secretstream state can't be restored mid-stream without replaying chunks to advance it), Photos chunked upload, and **iOS secretstream support** (chunk-uploaded files aren't readable on iOS until then — crypto-parity gate).
