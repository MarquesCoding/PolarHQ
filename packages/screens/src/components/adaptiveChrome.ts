...
type Probe = { ref: RefObject<HTMLElement | null>; set: (v: boolean | null) => void; last: boolean | null }
...
const luminanceBehind = (rect: DOMRect): number | null => {
  ...
  try {
    ...
    const s = (canvas.width = canvas.height = 10)
...
const tick = (now: number) => {
  if (now - lastSample >= 66) {
    ...
    for (const probe of probes) {
      const el = probe.ref.current
      const luma = el ? luminanceBehind(el.getBoundingClientRect()) : null
...
export const useContentLight = (ref: RefObject<HTMLElement | null>): boolean | null => {
  ...
  useEffect(() => {
    const probe: Probe = { ref, set: setLight, last: null }
    probes.add(probe)
    return () => {
      probes.delete(probe)
    }
