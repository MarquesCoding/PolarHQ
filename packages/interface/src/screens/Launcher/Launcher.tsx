...
import { fetchUsage } from "@polarhq/vault/photos"
import AppTile from "@polarhq/interface/screens/Launcher/components/AppTile/AppTile"
  ...
  return (
    <main className="mx-auto flex min-h-svh w-full max-w-5xl flex-col gap-8 p-6">
      <header className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">{sdkConfig().appName}</h1>
      ...
      <footer className="border-border/60 mt-auto flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <p className="text-muted-foreground/60 font-mono text-[10px] tracking-tight">
          {t("launcher.versionBuild", { version: sdkConfig().version, build: sdkConfig().build })}
        </p>
      </footer>
    </main>
  )
