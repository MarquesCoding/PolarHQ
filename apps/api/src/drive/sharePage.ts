...
export const renderSharePage = (view: ShareView, directUrl: string): string => {
  ...
  const e2eScript = `<script>(function(){var p=new URLSearchParams(location.hash.slice(1));var k=p.get('k');if(!k)return;var n=p.get('n');var fname=n?decodeURIComponent(n):'download';if(n){var ne=document.querySelector('.name');if(ne)ne.textContent=fname;document.title=fname+' · shared'}var btn=document.querySelector('.btn[href]');if(!btn)return;var url=btn.getAttribute('href');var mime=btn.getAttribute('data-mime')||'';btn.removeAttribute('href');btn.style.cursor='pointer';var sc=document.createElement('script');sc.src='/_sodium.js';sc.onerror=function(){fail(new Error('could not load crypto (/_sodium.js)'))};document.head.appendChild(sc);async function ready(){for(var i=0;i<600&&!window.sodium;i++){await new Promise(function(r){setTimeout(r,50)})}if(!window.sodium)throw new Error('crypto unavailable');await window.sodium.ready;return window.sodium}var objectUrl=null;async function getBlob(){if(objectUrl)return objectUrl;var s=await ready();var key=s.from_base64(decodeURIComponent(k),s.base64_variants.ORIGINAL);var resp=await fetch(url);if(!resp.ok)throw new Error('download failed ('+resp.status+')');var buf=new Uint8Array(await resp.arrayBuffer());var nb=s.crypto_secretbox_NONCEBYTES;var plain;try{plain=s.crypto_secretbox_open_easy(buf.slice(nb),buf.slice(0,nb),key)}catch(e){throw new Error('wrong key or corrupt data')}objectUrl=URL.createObjectURL(new Blob([plain],{type:mime}));return objectUrl}function save(){if(!objectUrl)return;var a=document.createElement('a');a.href=objectUrl;a.download=fname;document.body.appendChild(a);a.click();a.remove()}function showMedia(u){var el=mime.indexOf('video/')===0?document.createElement('video'):document.createElement('img');if(el.tagName==='VIDEO'){el.controls=true}el.src=u;el.style.cssText='width:100%;border-radius:12px;margin-top:18px;display:block';var ic=document.querySelector('.icon');if(ic)ic.style.display='none';btn.parentNode.insertBefore(el,btn)}var isMedia=mime.indexOf('image/')===0||mime.indexOf('video/')===0;function fail(e){btn.textContent=(e&&e.message)?('Failed: '+e.message):'Decryption failed';btn.removeAttribute('aria-disabled')}if(isMedia){btn.setAttribute('aria-disabled','true');btn.textContent='Decrypting…';getBlob().then(function(u){showMedia(u);btn.textContent='Download';btn.removeAttribute('aria-disabled')}).catch(fail);btn.addEventListener('click',function(e){e.preventDefault();save()})}else{btn.textContent='Decrypt & download';btn.addEventListener('click',function(e){e.preventDefault();if(objectUrl){save();return}btn.setAttribute('aria-disabled','true');btn.textContent='Decrypting…';getBlob().then(function(){save();btn.textContent='Download again';btn.removeAttribute('aria-disabled')}).catch(fail)})}})();</script>`
